...
ImageFactory& ImageFactory::getInstance() {
	static ImageFactory fact_instance;
...
ImageFactory::~ImageFactory() {
  ...
  //this one was done for us in class
  ...
  std::map<std::string, SDL_Texture*>::iterator tex_ptr = textures.begin();
  while (tex_ptr != textures.end()) {
	  SDL_DestroyTexture(tex_ptr->second);
	  tex_ptr++;
  }
  
  std::map<std::string, Image*>::iterator i_ptr = images.begin();
  while (i_ptr != images.end()) {
    std::cout << "deleting " << i_ptr->first << std::endl;
    delete i_ptr->second;	  
	i_ptr++;
  }
